...
class TestAnalyzeProjectWorkflow:
    ...
    def temp_project_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (project_dir / "src" / "components" / "App.jsx").write_text("export default function App() { return <div>App</div>; }")
...
"""
    ...
    def test_project_with_existing_adrs(self, temp_project_dir, temp_adr_dir, sample_existing_adr):
    ...
    def test_focus_areas_filtering(self, temp_project_dir, temp_adr_dir):
        ...
        assert "testing" in analysis_prompt.lower() or "build" in analysis_prompt.lower()
    ...
    def test_nonexistent_project_path(self, temp_adr_dir):
        ...
        assert any("not exist" in error.lower() or "not found" in error.lower() 
                  for error in result.errors)
        
        # Should have failed at the validation step
        failed_steps = [step for step in result.steps if step.status == WorkflowStatus.FAILED]
    ...
    def test_analysis_prompt_quality(self, temp_project_dir, temp_adr_dir):
        ...
        assert "look for" in prompt_lower or "examine" in prompt_lower or "analyze" in prompt_lower
