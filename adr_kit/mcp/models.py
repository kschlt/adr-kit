...
class MCPResponse(BaseModel):
    ...
    next_steps: List[str] = Field(default_factory=list, description="Suggested actions for agent")
...
class AnalyzeProjectRequest(BaseModel):
    ...
    focus_areas: List[str] = Field(default_factory=list, description="Specific areas to focus on (frontend, backend, database, etc.)")
...
class PreflightCheckRequest(BaseModel):
    ...
    context: Dict[str, Any] = Field(default_factory=dict, description="Additional context about the choice")
...
class CreateADRRequest(BaseModel):
    ...
    deciders: List[str] = Field(default_factory=list, description="People who made the decision")
    tags: List[str] = Field(default_factory=list, description="Tags for categorization")
    policy: Dict[str, Any] = Field(default_factory=dict, description="Structured policy block for enforcement")
...
class SupersedeADRRequest(BaseModel):
    ...
    new_consequences: str = Field(..., description="Expected outcomes of the new decision")
    supersede_reason: str = Field(..., description="Why the old ADR is being replaced")
    new_deciders: List[str] = Field(default_factory=list, description="Who made the new decision")
    new_tags: List[str] = Field(default_factory=list, description="Tags for the new ADR")
    new_policy: Dict[str, Any] = Field(default_factory=dict, description="Policy rules for the new decision")
    new_alternatives: Optional[str] = Field(None, description="Other options considered")
    auto_approve: bool = Field(False, description="Automatically approve new ADR without human review")
    adr_dir: str = Field("docs/adr", description="ADR directory path")
...
class PlanningContextRequest(BaseModel):
    ...
    domain_hints: List[str] = Field(default_factory=list, description="Domain hints (frontend, backend, database, etc.)")
...
class AnalyzeProjectData(BaseModel):
    ...
    analysis_prompt: str = Field(..., description="Specific questions to guide codebase analysis")
    project_context: Dict[str, Any] = Field(..., description="Technical stack details discovered")
    existing_adrs: List[str] = Field(..., description="ADRs already found in the project")
    suggested_decisions: List[str] = Field(default_factory=list, description="Potential decisions needing ADRs")
...
class PreflightCheckData(BaseModel):
    ...
    conflicting_adrs: List[str] = Field(default_factory=list, description="ADR IDs that conflict with this choice")
    related_adrs: List[str] = Field(default_factory=list, description="ADR IDs that are related but don't conflict")
...
class CreateADRData(BaseModel):
    ...
    conflicts: List[str] = Field(default_factory=list, description="Any conflicting ADRs detected")
    related_adrs: List[str] = Field(default_factory=list, description="Similar existing ADRs found")
    validation_warnings: List[str] = Field(default_factory=list, description="Validation warnings if any")


class ApproveADRData(BaseModel):
    """Data returned by adr_approve tool."""
    adr_id: str = Field(..., description="The approved ADR ID")
    status: str = Field("approved", description="Always 'approved' after successful approval")
    policies_activated: List[str] = Field(default_factory=list, description="List of policy rules now active")
    configurations_updated: List[str] = Field(default_factory=list, description="Config files that were updated")
    warnings: List[str] = Field(default_factory=list, description="Any issues encountered during automation")
...
class SupersedeADRData(BaseModel):
    ...
    new_status: str = Field(..., description="Status of the new ADR (usually 'proposed')")
    relationships_updated: List[str] = Field(default_factory=list, description="What links were updated")


class PlanningContextData(BaseModel):
    """Data returned by adr_planning_context tool."""
    relevant_adrs: List[Dict[str, Any]] = Field(default_factory=list, description="ADRs that apply to your task")
    constraints: List[str] = Field(default_factory=list, description="Hard restrictions from approved ADRs")
    guidance: str = Field("", description="Specific advice for your task context")
    use_technologies: List[str] = Field(default_factory=list, description="Technologies recommended for your task")
    avoid_technologies: List[str] = Field(default_factory=list, description="Technologies to avoid based on ADRs")
    patterns: List[str] = Field(default_factory=list, description="Architectural patterns suggested for your task")
    checklist: List[str] = Field(default_factory=list, description="Steps to ensure ADR compliance")
