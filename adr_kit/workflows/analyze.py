...
class AnalyzeProjectWorkflow(BaseWorkflow):
    ...
    def _scan_project_structure(self, project_root: Path) -> Dict[str, Any]:
        ...
        config_patterns = [
            ...
            ".eslintrc*", "tsconfig.json", "webpack.config.*",
            "Dockerfile", "docker-compose.yml", ".env*"
        ]
        
        common_dir_patterns = [
            "src", "lib", "app", "components", "services", "utils",
            ...
            for root, dirs, files in os.walk(project_root):
                ...
                for dir_name in dirs:
                    if dir_name.lower() in common_dir_patterns:
                        structure["common_directories"].append(str(Path(root) / dir_name))
    ...
    def _detect_technologies(self, project_root: Path, structure: Dict[str, Any]) -> Dict[str, Any]:
        ...
        tech_patterns = {
            ...
            "Express.js": {
                "files": ["package.json"],
                "content_patterns": ['"express":', 'require("express")', 'import express']
            },
            "FastAPI": {
            ...
            "MySQL": {
                "content_patterns": ["mysql", "pymysql", "mysql2"]
            },
            "MongoDB": {
                "content_patterns": ["mongodb", "mongoose", "pymongo"]
            },
            "Redis": {
                "content_patterns": ["redis", "ioredis"]
            },
        ...
        for tech_name, patterns in tech_patterns.items():
            ...
            if "extensions" in patterns:
                for ext in patterns["extensions"]:
                    if ext in structure["file_types"] and structure["file_types"][ext] > 0:
    ...
    def _generate_analysis_prompt(
        ...
        prompt_parts = [
            ...
            f"- Existing ADRs: {adr_count} found" if adr_count > 0 else "- No existing ADRs found",
            ""
        ]
        ...
        if technologies:
            prompt_parts.extend([
                "**Technology-Specific Areas to Examine:**"
            ])
            
            for tech in technologies:
                if tech in ["React", "Vue", "Angular"]:
                    prompt_parts.append(f"- {tech}: Component architecture, state management, routing decisions")
                elif tech in ["Express.js", "FastAPI", "Django", "Flask"]:
                    prompt_parts.append(f"- {tech}: API design patterns, middleware choices, authentication strategy")
                elif tech in ["PostgreSQL", "MySQL", "MongoDB"]:
                    prompt_parts.append(f"- {tech}: Database schema design, migration strategy, connection patterns")
                elif tech == "TypeScript":
                    prompt_parts.append(f"- {tech}: Type system usage, configuration choices, strict mode settings")
                elif tech == "Docker":
                    prompt_parts.append(f"- {tech}: Containerization strategy, multi-stage builds, orchestration")
        ...
        if any(tech in technologies for tech in ["Express.js", "FastAPI", "Django", "Flask"]):
            suggested_focus.append("api_design")
        if any(tech in technologies for tech in ["PostgreSQL", "MySQL", "MongoDB", "Redis"]):
